...
// Functions + Operators: Logical (&&), Comparison (!=)
function validatePasswordStrength($pwd) {
    return strlen($pwd) >= 8 && preg_match('/[A-Za-z]/', $pwd) && preg_match('/[0-9]/', $pwd);
}

// Classes (OOP): Simple User class
class User {
    private $name;
...
// Database Connectivity: Fetch roles
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($full_name) || empty($password) || $role_id == 0) {
        $errors[] = "All fields are required!";
    } elseif (!validatePasswordStrength($password)) {
        $errors[] = "Password ≥ 8 chars, letters + numbers required.";
    } else {
        ...
        if ($stmt->fetch()) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (full_name, password, role_id) VALUES (?, ?, ?)");
            $stmt->execute([$full_name, $hash, $role_id]);
...
<head>
    <meta charset="UTF-8">
    <title>Sign Up - Event Portal</title>
    ...
    <style>
        ...
        @media (max-width: 640px) {
            ...
            .form-input, select { padding: 0.75rem; }
...
<body>
    ...
    <div class="card">
        <div class="text-center mb-6">
            ...
            <p class="text-gray-300">Join the Event Management Portal</p>
        </div>
        ...
        <form method="POST" class="space-y-5">
            ...
            <div>
                <label class="block font-medium flex items-center"><i class="fas fa-users icon"></i> Role</label>
            ...
            <button type="submit" class="btn-primary"><i class="fas fa-sign-in-alt mr-2"></i>Sign Up</button>
        </form>
        <p class="text-center mt-4 text-gray-300">Already have an account? <a href="login.php">Login</a></p>
    </div>
</body>
</html>