...
if ($view === 'my') {
    ...
    $stmt = $conn->prepare("
        ...
        FROM events e
        JOIN registrations r ON e.event_id = r.event_id
        WHERE r.user_id = ? AND r.status = 'approved' AND e.event_time < NOW()
        ORDER BY e.event_time DESC");
    $stmt->execute([$user_id]);
    $events = $stmt->fetchAll();
...
<body>
    ...
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="?view=my" class="<?= $view==='my'?'active':'' ?>">
                <i class="fas fa-calendar-check mr-2"></i>My Registrations
            </a>
            <a href="?view=feedback" class="<?= $view==='feedback'?'active':'' ?>">
    ...
    <div class="content">
        <div class="container">
            <h1 class="text-3xl font-bold mb-6">
                <i class="fas fa-user mr-2"></i>Welcome, <?= htmlspecialchars($user->getName()) ?>
            </h1>

            <?php if (empty($events)): ?>
                <div class="bg-white p-8 rounded-lg shadow text-center">
                    <p class="text-gray-600 text-lg">No events to display.</p>
                </div>
            <?php else: ?>
                        ...
                        <div class="event-card">
                            ...
                            <div class="p-6">
                                ...
                                <p class="text-gray-600 mt-2">
                                    <i class="fas fa-map-marker-alt mr-2"></i><?= htmlspecialchars($e['location']) ?>
                                </p>
                                <p class="text-gray-600">
                                    <i class="fas fa-clock mr-2"></i><?= date('d M Y, h:i A', strtotime($e['event_time'])) ?>
                                </p>
                                ...
                                <?php if ($view === 'feedback'): ?>
                                    ...
                                    $eventPassed = strtotime($e['event_time']) < time();
                                    ?>
                                    <?php if ($hasFeedback): ?>
