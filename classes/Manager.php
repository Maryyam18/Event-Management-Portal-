...
class Manager extends User {
    ...
    public function save() {
        $stmt = $this->pdo->prepare("INSERT INTO users (full_name, email, password, role_id) VALUES (:name, :email, :password, :role_id)");
        $stmt->execute([
            ':name' => $this->full_name,
            ':email' => $this->email,
    ...
    public function update($id, $full_name, $email, $password) {
        $stmt = $this->pdo->prepare("UPDATE users SET full_name = :name, email = :email, password = :password WHERE user_id = :id");
        $stmt->execute([
            ':name' => $full_name,
            ':email' => $email,
            ':password' => $password,
